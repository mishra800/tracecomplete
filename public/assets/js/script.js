(function ($) {
  ...
  if ($(".team-two__carousel").length) {
    $(".team-two__carousel").owlCarousel({
      ...
      autoplay: true,
      autoplayTimeout: 7000,
      startPosition: 0, // Always start with the first item (Founder & Managing Director)
      navText: [
        '<span class="icon-right-up"></span>',
        '<span class="icon-right-up"></span>',
      ],
  ...
  // Fix for stuck locked class - ensure body can scroll on page load
  $(document).ready(function() {
    // Remove locked class on page load to ensure scrolling works
    ...
    // Also remove it when clicking outside mobile nav
    $(document).on('click', function(e) {
      if (!$(e.target).closest('.mobile-nav__wrapper, .mobile-nav__toggler').length) {
    ...
    // Fix for any stuck overflow hidden
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('a[href*="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            // Split href to handle cross-page navigation
            const [page, hash] = this.getAttribute('href').split('#');
            const currentPage = window.location.pathname.split('/').pop() || 'index.html';

            if (hash) {
                // If the link is on the same page or no page is specified
                if (!page || page === currentPage || page === '/') {
                    const targetElement = document.getElementById(hash);
                    if (targetElement) {
                        // Get the header height (adjust selectors as needed)
                        const header = document.querySelector('.main-header-two') || document.querySelector('.stricky-header');
    ...
    if (window.location.hash) {
        ...
        if (targetElement) {
            ...
            // Scroll to the target position after a slight delay to ensure page load
            setTimeout(() => {
                window.scrollTo({
                    top: targetPosition,
                    behavior: 'smooth'
                });
            }, 100); // 100ms delay to allow page rendering
        }
    }
});